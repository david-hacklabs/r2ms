...
if (isset ( $_POST ['delete_project'] )) {
	...
	if (is_int ( $value )) {
		...
		if ($value == 0) {
			...
			if (delete_value ( "projects", $value )){
				// Audit log
				$risk_id = 1000;
...
$projects = get_projects (); // Get the total number of projects
$count = count ( $projects ); // Initialize the counter
$counter = 1; // For each project created
foreach ( $projects as $project ) { 
	// Get the project ID 
	$id = $project ['value'];
	echo "#sortable-" . $id . " li"; // If it's not the	last one 
	if ($counter != $count) {
		echo ", ";
		$counter ++;
	}
						...
						<div class="span12">
							<div class="hero-unit">
								...
								<form name="project" method="post" action="">
									<p>
                <?php echo $lang['AddNewProjectNamed']; ?> <input
											name="new_project" type="text" maxlength="100" size="20" />&nbsp;&nbsp;<input
											type="submit" value="<?php echo $lang['Add']; ?>"
											name="add_project" /><br />
                <?php echo $lang['DeleteCurrentProjectNamed']; ?> <?php create_dropdown("projects"); ?>&nbsp;&nbsp;<input
