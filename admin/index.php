...
if (isset ( $_POST ['update_risk_levels'] )) {
	...
	if (is_int($critical) && is_int ( $high ) && is_int ( $medium ) && is_int ( $low ) && is_int ( $risk_model )) {
		// Check if low < medium < high
		if (($low < $medium) && ($medium < $high) && ($high < $critical)) {
			// Update the risk level
			if (update_risk_levels ( $critical, $high, $medium, $low )) {
				// Audit log
				$risk_id = 1000;
		...
		if ((1 <= $risk_model) && ($risk_model <= 6)) {
			...
			if (update_risk_model ( $risk_model )){
				// Audit log
				$risk_id = 1000;
		...
		<div class="row-fluid">
			<div class="span3">
				<ul class="nav  nav-pills nav-stacked">
					<li class="active"><a href="index.php"><?php echo $lang['ConfigureRiskFormula']; ?></a>
					</li>
			...
			<div class="span9">
				...
				<p><?php echo $lang['IConsiderCriticalRiskToBeAnythingGreaterThan']; ?>: <input
										type="text" name="critical" size="2"
										value="<?php echo $risk_levels[3]['value']; ?>" />
								</p>
				
                <p><?php echo $lang['IConsiderHighRiskToBeAnythingGreaterThan']; ?>: <input
										type="text" name="high" size="2"
										value="<?php echo $risk_levels[2]['value']; ?>" />
								</p>
								<p><?php echo $lang['IConsiderMediumRiskToBeLessThanAboveButGreaterThan']; ?>: <input
										type="text" name="medium" size="2"
										value="<?php echo $risk_levels[1]['value']; ?>" />
								</p>
								<p><?php echo $lang['IConsiderlowRiskToBeLessThanAboveButGreaterThan']; ?>: <input
										type="text" name="low" size="2"
										value="<?php echo $risk_levels[0]['value']; ?>" />
								</p>

								<input type="submit" value="<?php echo $lang['Update']; ?>"
									name="update_risk_levels" />
