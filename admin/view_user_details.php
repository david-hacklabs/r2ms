...
if (isset ( $_POST ['update_user'] ) && isset ( $_POST ['user'] )) {
	...
	if (is_int ( $user_id )) {
		...
		if ($none)
			...
			// Update the user
		if (update_user ( $user_id, $name, $email, $team, $language, $admin, $review_critical, $review_high, $review_medium, $review_low, $submit_risks, $modify_risks, $plan_mitigations, $close_risks, $multi_factor )){
			// Audit log
			$risk_id = 1000;
...
if (isset ( $_POST ['user'] )) {
	...
	$review_critical = $user_info ['review_critical'];
...
?>
    <div class="container-fluid">
		...
		<?php echo $lang['Type']; ?>: <input style="cursor: default;"
									name="type" type="text" maxlength="20" size="20"
									title="<?= get_name_by_value("typeuser", $type); ?>" disabled="disabled"
									value="<?= get_name_by_value("typeuser", $type); ?>" /><br />
		<?php echo $lang['Company']; ?>: <input style="cursor: default;"
									name="company_name" type="text" maxlength="20" size="20"
									title="<?=get_name_by_value("company", $company_id); ?>" disabled="disabled"
									value="<?=get_name_by_value("company", $company_id); ?>"/> </br>							
                <?php echo $lang['FullName']; ?>: <input name="name"
									type="text" maxlength="50" size="20"
									value="<?php echo htmlentities($name, ENT_QUOTES, 'UTF-8'); ?>" /><br />
                <?php echo $lang['EmailAddress']; ?>: <input
									name="email" type="text" maxlength="200" size="20"
									value="<?php echo htmlentities($email, ENT_QUOTES, 'UTF-8'); ?>" /><br />
                <?php echo $lang['Username']; ?>: <input
									style="cursor: default;" name="username" type="text"
									maxlength="20" size="20"
									title="<?php echo htmlentities($username, ENT_QUOTES, 'UTF-8'); ?>"
									disabled="disabled"
									value="<?php echo htmlentities($username, ENT_QUOTES, 'UTF-8'); ?>" /><br />
		<?php echo $lang['LastLogin']; ?>: <input style="cursor: default;"
									name="last_login" type="text" maxlength="20" size="20"
									title="<?php echo $last_login; ?>" disabled="disabled"
									value="<?php echo $last_login; ?>" /><br />
                <?php echo $lang['Language']; ?>: <?php create_dropdown("languages", get_value_by_name("languages", $language)); ?>
                <h6>
									<u><?php echo $lang['Teams']; ?></u>
								</h6>
                <?php create_multiple_dropdown("team", $teams, null, true); ?>
                <h6>
									<u><?php echo $lang['UserResponsibilities']; ?></u>
								</h6>
								<ul>
									<li><input name="submit_risks" type="checkbox"
										<?php if ($submit_risks) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToSubmitNewRisks']; ?></li>
									<li><input name="modify_risks" type="checkbox"
										<?php if ($modify_risks) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToModifyExistingRisks']; ?></li>
									<li><input name="close_risks" type="checkbox"
										<?php if ($close_risks) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToCloseRisks']; ?></li>
									<li><input name="plan_mitigations" type="checkbox"
										<?php if ($plan_mitigations) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToPlanMitigations']; ?></li>
									<li><input name="review_low" type="checkbox"
										<?php if ($review_low) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToReviewLowRisks']; ?></li>
									<li><input name="review_medium" type="checkbox"
										<?php if ($review_medium) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToReviewMediumRisks']; ?></li>
									<li><input name="review_high" type="checkbox"
										<?php if ($review_high) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToReviewHighRisks']; ?></li>
									<li><input name="review_critical" type="checkbox"
										<?php if ($review_critical) echo " checked" ?> />&nbsp;<?php echo $lang['AbleToReviewCriticalRisks']; ?></li>
									<li><input name="admin" type="checkbox"
										<?php if ($admin) echo " checked" ?> />&nbsp;<?php echo $lang['AllowAccessToConfigureMenu']; ?></li>
								</ul>
                <input class="btn btn-primary" type="submit"
									value="<?php echo $lang['Update']; ?>" name="update_user" /><br />
								</p>
							</form>
						</div>
					</div>
