...
if (isset($company) && existNameByValue('company', $company) && isset($project) && existNameByValue('project_version', $project)){
	if (!projectversion_exist_in_company($project, $company)){
		...
		$return->message = "<p class='redalert'>The application cannot find this version project associated to the company.</p>";
		
		// Audit log
		$risk_id = 1000;
		$message = "An existing project is not able to be found in order to show the trend of the risks presented in the requested project by the \"" . $_SESSION ['user'] . "\" user.";
		write_log ( $risk_id, $_SESSION ['uid'], $message );
	}
	else {
		$return->message = new stdClass();
		$return->message->risk_trend = get_risk_trend($project);
				
		$return->success = true;
	
		// Audit log
		$risk_id = 1000;
		$message = "An existing project is able to be found in order to show the trend of the risks presented in the requested project \"" . $_SESSION ['user'] . "\" user.";
		write_log ( $risk_id, $_SESSION ['uid'], $message );
	}
}
else {
	...
	// Audit log
	$risk_id = 1000;
	$message = "An AJAX request to get the trend of the risks for a specific project fails by the \"" . $_SESSION ['user'] . "\" user.";
	write_log ( $risk_id, $_SESSION ['uid'], $message );
}
