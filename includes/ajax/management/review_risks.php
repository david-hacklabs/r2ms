...
if (isset($company) && existNameByValue('company', $company) && isset($project) && existNameByValue('project_version', $project)){
	if (!projectversion_exist_in_company($project, $company)){
		...
		// Audit log
		$risk_id = 1000;
		$message = "An existing version project is not be able to be found in order to show the require management review by the \"" . $_SESSION ['user'] . "\" user.";
		write_log ( $risk_id, $_SESSION ['uid'], $message );
	}
	else {
		
		$tablecontent = get_reviews_table($project, 3);
		...
		// Audit log
		$risk_id = 1000;
		$message = "An existing version project is able to be found in order to show the require management review by the \"" . $_SESSION ['user'] . "\" user.";
		write_log ( $risk_id, $_SESSION ['uid'], $message );
	}
}
else {
	...
	// Audit log
	$risk_id = 1000;
	$message = "An AJAX request to get the management review fail by the \"" . $_SESSION ['user'] . "\" user.";
	write_log ( $risk_id, $_SESSION ['uid'], $message );
}
