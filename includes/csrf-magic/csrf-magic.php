 ...
 * and exit if the CSRF token is invalid.
 * This will defer executing
 ...
 * $fatal == true (see csrf_check).
 * This will usually output an error message
 * about the failure.
 */
 ...
 * AJAX requests on this domain.
 * Set this to the web path. This setting only works
...
/**
 * A secret key used when hashing items.
 * Please generate a random string and
 ...
 * its rewriting capabilities.
 * If you're serving non HTML content, you should
 * definitely set this false.
 */
$GLOBALS ['csrf'] ['rewrite'] = true;

/**
 * Whether or not to use IP addresses when binding a user to a token.
 * This is
 ...
 * This is a shortcut implementation
 ...
 * application.
 * The token will then be globally valid for all of that user's
 * operations, but no one else. This requires that 'secret' be set.
 */
$GLOBALS ['csrf'] ['user'] = false;

/**
 * This is an arbitrary secret value associated with the user's session.
 * This
 ...
 * 
 * @param bool $fatal
 *        	Whether or not to fatally error out if there is a problem.
...
/**
 * Retrieves a valid token(s) for a particular context.
...
/**
 * Checks if a composite token is valid.
 * Outward facing code should use this
...
/**
 * Generates a hash/expiry double.
 * If time isn't set it will be calculated
